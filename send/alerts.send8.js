(function($){
	...
	$(function(){
		...
		var tags = $('#tags');
		var sendToTypes = [5,21]; // define the types that can be sent-to
				...
				if ( sports[n].notifications && sports[n].notifications.sport ){
					...
					for ( var o=0;o<sports[n].notifications.sport.length;o++){
						if ( sports[n].notifications.sport[o] ){
							if ( $.inArray(sports[n].notifications.sport[o].type, sendToTypes) > -1 && (sports[n].notifications.sport[o].desc.toLowerCase().indexOf(term) > -1) ){
								l = sports[n].notifications.sport[o].desc;
					...
					for (var i=0;i<leagues.length;i++){
						...
						if ( leagueOptions ){
							for (var a=0;a<leagueOptions.length;a++){
								if ( $.inArray(leagueOptions[a].type, sendToTypes) > -1 && (leagueOptions[a].desc.toLowerCase().indexOf(term) > -1 || leagueName.toLowerCase().indexOf(term) > -1) ){
									cat = leagueOptions[a].category||category;
									l = leagueOptions[a].desc;
		...
		if ( espn.alerts.categories ){
			...
			for (var n=0;n<sports.length;n++){
				if ( sports[n].notifications && sports[n].notifications.sport ){
					...
					for ( var o=0;o<sports[n].notifications.sport.length;o++){
						if ( sports[n].notifications.sport[o] ){
							txt = sports[n].notifications.sport[o].desc;
							if (sports[n].notifications.sport[o].category){
		...
		espn.alerts.loadAndWriteStatsAndMessages({type:5},'#alerts-recent','#alerts-graph');
